...
public abstract class EntityPlayer extends EntityLiving
{
    ...
    public boolean attackEntityFrom(Entity entity, int i)
    {
        ...
        func_25046_a(StatList.field_25100_t, i);
    ...
    protected void func_25047_a(EntityLiving entityliving, boolean flag)
    {
        ...
        if((entityliving instanceof EntityPlayer) && !func_27016_t())
        {
            return;
        }
    ...
    private void func_25045_g(double d, double d1, double d2)
    {
        ...
        if(isOnLadder())
        {
            if(d1 > 0.0D)
            {
                func_25046_a(StatList.field_25110_n, (int)Math.round(d1 * 100D));
            }
        } else
        if(onGround)
        {
        ...
        if(ridingEntity != null)
        {
            ...
            if(i > 0)
            {
                if(ridingEntity instanceof EntityMinecart)
                {
                    ...
                    if(field_9155_d.func_27127_a(MathHelper.floor_double(posX), MathHelper.floor_double(posY), MathHelper.floor_double(posZ)) >= 1000D)
                    {
    ...
    protected void fall(float f)
    {
        if(f >= 2.0F)
        {
            func_25046_a(StatList.field_25111_m, (int)Math.round((double)f * 100D));
        }
        super.fall(f);
    }
