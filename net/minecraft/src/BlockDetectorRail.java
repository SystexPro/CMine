...
public class BlockDetectorRail extends BlockRail
{
    ...
    private void func_27035_f(World world, int i, int j, int k, int l)
    {
        ...
        boolean flag1 = false;
        float f = 0.125F;
        List list = world.getEntitiesWithinAABB(EntityMinecart.class, AxisAlignedBB.getBoundingBoxFromPool((float)i + f, j, (float)k + f, (float)(i + 1) - f, (double)j + 0.25D, (float)(k + 1) - f));
        if(list.size() > 0)
        {
            flag1 = true;
        }
